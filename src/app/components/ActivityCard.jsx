    ...
    return (
        <div key={`div-1-${activityId}`} className="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
            ...
            <h5 key={`h5-1-${activityId}`} className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{title}</h5>
                <p key={`p-1-${activityId}`} className="text-lg text-gray-900 dark:text-white">{dateTime.split("T")[0]}</p>
                <Link key={`a-3-${activityId}`} href={`/activities/${activityId}`} className="inline-flex items-center mt-3 px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                    Read more
