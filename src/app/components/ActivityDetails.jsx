...
const ActivityDetails = ({ title, dateTime, description, picsUrls, videoUrls }) => {
    ...
    return (
        setting && <div className="text-left grid gap-3 p-3">
            <div className={`bg-black flex flex-col w-full h-svh z-50 top-0 left-0 fixed  ${isScaleOpen ? "block" : "hidden"}`}>
                ...
                <div className="h-5/6 bg-white w-auto m-auto">
                    {scaleImageUrl && <img className="h-full w-auto rounded-lg" src={scaleImageUrl} alt="activity picture" />}
                </div>
            </div>
